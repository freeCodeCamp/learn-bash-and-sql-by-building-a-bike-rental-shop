...
describe('The "rentals" table', () => {
  it('should have a "bike_id" column as a foreign key that references the correct column', async () => {
    ...
    try {
      ...
      const foreignKeyColumn = res.rows.findIndex(row => {
        return row.table_name === 'rentals' && row.column_name === 'bike_id' && row.foreign_table_name === 'bikes' && row.foreign_column_name === 'bike_id';
      });
