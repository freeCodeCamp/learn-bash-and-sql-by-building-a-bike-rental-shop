...
CONNECT="psql -X --username=freecodecamp --dbname=bikes --no-align --tuples-only -c"
...
RENT_MENU() {
  SHOW_TITLE
  AVAILABLE_BIKES=$($CONNECT "SELECT bike_id, type, size FROM bikes WHERE available=TRUE ORDER BY bike_id;")
  ...
  then
    ...
    BIKE_AVAILABILITY=$($CONNECT "SELECT available FROM bikes WHERE bike_id=$BIKE_ID_TO_RENT;")
    ...
    # Bike is available
      ...
      CUSTOMER_NAME=$($CONNECT "SELECT name FROM customers WHERE phone='$PHONE_NUMBER';")
      ...
      then
        ...
        # Insert new customer
        $($CONNECT "INSERT INTO customers(name, phone) values('$CUSTOMER_NAME', '$PHONE_NUMBER');")
      fi

      CUSTOMER_ID=$($CONNECT "SELECT customer_id FROM customers WHERE phone='$PHONE_NUMBER';")
      ...
      BIKE_TYPE=$($CONNECT "SELECT type FROM bikes WHERE bike_id=$BIKE_ID_TO_RENT;")
      BIKE_SIZE=$($CONNECT "SELECT size FROM bikes WHERE bike_id=$BIKE_ID_TO_RENT;")
...
RETURN_MENU() {
  ...
  CUSTOMERS_RENTALS=$($CONNECT "SELECT bikes.bike_id, type, size FROM bikes LEFT OUTER JOIN rentals ON bikes.bike_id = rentals.bike_id LEFT OUTER JOIN customers ON customers.customer_id = rentals.customer_id WHERE phone='$PHONE_NUMBER' AND date_returned is null;")
  ...
  then
    ...
    CUSTOMER_HAS_BIKE=$($CONNECT "SELECT * FROM rentals LEFT OUTER JOIN customers ON rentals.customer_id = customers.customer_id WHERE phone='$PHONE_NUMBER' AND date_returned is null AND bike_id=$BIKE_ID_TO_RETURN;")

    if [[ -z $CUSTOMER_HAS_BIKE ]]
    then
      echo "You do not have that bike."
    else
      RETURN_BIKE_RESULT=$($CONNECT "UPDATE rentals SET date_returned=now() WHERE bike_id=$BIKE_ID_TO_RETURN;")
